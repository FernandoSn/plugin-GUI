...
int GetNameOfBoard(int TestBoard, char* BoardName)
{
	...
	ULStat = MCDAQ::cbGetBoardName(TestBoard, BoardName);
	if (ULStat != NOERRORS) {
		DisplayMessage(ULStat);
		retVal = 0;
	}
...
void DisplayMessage(int ErrorNumber)
{
	...
	if (ErrorNumber != NOERRORS) {
		ULStat = MCDAQ::cbGetErrMsg(ErrorNumber, ErrMessage);
		printf("\a\n");
