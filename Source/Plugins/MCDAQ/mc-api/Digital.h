	...
	int* DefaultNumBits, int* FirstBit)
{
	...
	*DefaultNumBits = 0;
	...
	ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, BIDINUMDEVS, &NumPorts);
	if (ULStat != NOERRORS) {
		NumPorts = 0;
		return 0;
	}
	...
	if ((PortType == PORTOUTSCAN) || (PortType == PORTINSCAN)) {
		...
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, BI_DISCANOPTIONS, &digInScanOptions);
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, BI_DOSCANOPTIONS, &digOutScanOptions);
		// Get daq input scan options, then daq output scan options
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, BI_DAQISCANOPTIONS, &daqInScanOptions);
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, BI_DAQOSCANOPTIONS, &daqOutScanOptions);
		...
		if (PortType == PORTDAQINSCAN)
			if (daqInScanOptions == 0)
				...
				ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, BI_DAQINUMCHANTYPES, &configVal);
				// Get the port types available for DaqInScan
				for (chan = 0; chan < configVal; chan++) {
					ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, chan, BI_DAQICHANTYPE, &configVal);
		...
		if (PortType == PORTDAQOUTSCAN)
			if (daqOutScanOptions == 0)
				...
				//Get the number of Daq input channels
				ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, 378, &configVal);
				// Get the port types available for DaqInScan
				for (chan = 0; chan < configVal; chan++) {
					ULStat = MCDAQ::cbGetConfig(BOARDINFO, BoardNum, 0, 379, &configVal);
					if ((configVal == DIGITAL8) | (configVal == DIGITAL16) | (configVal == DIGITAL)) {
						DaqDigitalExists = TRUE;
						break;
					}
	...
	for (DioDev = 0; DioDev < NumPorts; ++DioDev) {
		progAbility = -1;
		ULStat = MCDAQ::cbGetConfig(DIGITALINFO, BoardNum, DioDev, DIINMASK, &InMask);
		ULStat = MCDAQ::cbGetConfig(DIGITALINFO, BoardNum, DioDev, DIOUTMASK, &OutMask);
		if ((InMask & OutMask) > 0)
			progAbility = FIXEDPORT;
		ULStat = MCDAQ::cbGetConfig(DIGITALINFO, BoardNum, DioDev, DIDEVTYPE, &ThisType);
		if (ULStat == NOERRORS)
			CurPort = ThisType;
		...
		if (!PortIsCompatible) {
			if (progAbility != FIXEDPORT) {
				...
				if ((CurPort == AUXPORT) && CheckBitProg) {
					...
					ULStat = MCDAQ::cbDConfigBit(BoardNum, AUXPORT, firstBit, ConfigDirection);
					if (ULStat == NOERRORS) {
						//return port to input mode
						ULStat = MCDAQ::cbDConfigBit(BoardNum, AUXPORT, firstBit, DIGITALIN);
						progAbility = PROGBIT;
					} else {
				...
				if (progAbility == -1) {
					...
					ULStat = MCDAQ::cbDConfigPort(BoardNum, CurPort, ConfigDirection);
					if (ULStat == NOERRORS) {
						//return port to input mode
						ULStat = MCDAQ::cbDConfigBit(BoardNum, AUXPORT, firstBit, DIGITALIN);
		...
		if (PortIsCompatible)
			PortsFound += 1;
		if (*DefaultPort == -1) {
			ULStat = MCDAQ::cbGetConfig(DIGITALINFO, BoardNum, DioDev, DINUMBITS, &NumBits);
			if (progAbility == FIXEDPORT) {
				...
				do {
					BitWeight = (int)pow((float)2, CurBit);
	...
	ULStat = MCDAQ::cbErrHandling(ReportError, HandleError);
	*FirstBit = firstBit;
