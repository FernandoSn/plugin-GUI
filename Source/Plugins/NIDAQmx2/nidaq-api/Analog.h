	...
	int* DefaultRange, int* DefaultChan, int* DefaultTrig)
{
	...
	*DefaultChan = 0;
	...
	case VOLTAGEINPUT:
		...
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BINUMADCHANS, &ChansFound);
		if (ULStat != NOERRORS) {
		...
		if (ChansFound > 0) {
			...
			ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BIADRES, &resAD);
			if (ULStat == NOERRORS) {
				*Resolution = resAD;
				ADRes = resAD;
			}
			...
			ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BIRANGE, &HardRange);
			...
			if (IOType == ANALOGDAQIN) {
				...
				ULStat = MCDAQ::cbGetIOStatus(BoardNum, &status, &curCount, &curIndex, functionType);
				if (ULStat != NOERRORS) {
					return 0;
				}
			}
		...
		break;
	case TEMPERATUREIN:
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BINUMTEMPCHANS, &ConfigVal);
		if (!ConfigVal)
			return 0;
		return ConfigVal;
	default:
		// Get the number of D/A channels
		ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BINUMDACHANS, &ChansFound);
		if (ULStat != NOERRORS) {
			DisplayMessage(ULStat);
			return ChansFound;
		}
		if (ChansFound > 0) {
			ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BIDACRES, &DARes);
			*Resolution = DARes;
			...
			if (IOType == ANALOGDAQOUT)
			{
				...
				ULStat = MCDAQ::cbGetStatus(BoardNum, &status, &curCount, &curIndex, functionType);
				if (ULStat != NOERRORS) {
					return 0;
				}
			}
	...
	{
		ULStat = MCDAQ::cbSetTrigger(TestBoard, TRIGABOVE, 0, 0);
		if (ULStat == NOERRORS) {
	...
	if ((ChansFound > 0) & CheckPretrig)
	{
		...
		ULStat = MCDAQ::cbDaqSetTrigger(TestBoard, TRIG_IMMEDIATE,
			TRIGABOVE, 0, ANALOG, *DefaultRange, 0.0f, 0.1f, START_EVENT);
		if (ULStat == NOERRORS)
			*DefaultTrig = TRIGABOVE;
		else {
			ULStat = MCDAQ::cbSetTrigger(TestBoard, TRIG_POS_EDGE, 0, 0);
			if (ULStat == NOERRORS)
				*DefaultTrig = TRIG_POS_EDGE;
			else
				ChansFound = 0;
	...
	for (TestRange = BIP5VOLTS; TestRange <= BIP30VOLTS; TestRange++)
	{
		if (ADRes > 16) {
			if ((IOType == ANALOGINPUT) | (IOType == ANALOGDAQIN)) {
				ULStat = MCDAQ::cbAIn32(TestBoard, 0, TestRange, &dataHRValue, Options);
				if (ULStat == BADBOARDTYPE) {
					ULStat = MCDAQ::cbVIn32(TestBoard, 0, 0, &dataDblValue, 0);
			...
			if ((IOType == ANALOGINPUT) | (IOType == ANALOGDAQIN)) {
				ULStat = MCDAQ::cbAIn(TestBoard, 0, TestRange, &dataValue);
				if (ULStat == BADBOARDTYPE) {
					ULStat = MCDAQ::cbVIn(TestBoard, 0, 0, &dataFltValue, 0);
					printf("This device is not compatible with this function.\n\n");
					if (ULStat == NOERRORS)
						printf("   Use 'VIn01' example instead.\n");
					DisplayMessage(BADBOARDTYPE);
...
int* TestOutputRanges(int* DefaultRange, int* numFound)
{
	...
	*DefaultRange = NOTUSED;
	...
	for (TestRange = BIP5VOLTS; TestRange <= BIP30VOLTS; TestRange++)
	{
		ULStat = MCDAQ::cbAOut(TestBoard, 0, TestRange, dataValue);
		if (ULStat == NOERRORS) {
			ULStat = MCDAQ::cbGetConfig(2, TestBoard, 0, BIDACRANGE, &configVal);
			if (ULStat == NOERRORS) {
				// fixed range
...
void GetTrigResolution(int* tRes, int* tRange)
{
	...
	ATrigRange = 0;
	ATrigRes = 0;
	ULStat = MCDAQ::cbGetConfig(TestBoard, 2, 0, BIADTRIGSRC, &TrigSource);
	ULStat = MCDAQ::cbGetConfig(BOARDINFO, TestBoard, 0, BIBOARDTYPE, &BoardID);
	if (ULStat == NOERRORS) {
		switch (BoardID) {
		case 95:
		case 96:
